...
int CBarnacle::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
...
// barnacle has prey.
	...
	// prey is lifted fully into feeding position and is dangling there.
			...
			if ( m_flKillVictimTime != -1 && gpGlobals->time > m_flKillVictimTime )
			{
				...
				if ( pVictim )
				{
					pVictim->TakeDamage ( pev, pev, pVictim->pev->health, ID_DMG_BARNACLE_CHOMP );
					m_cGibs = 3;
				}

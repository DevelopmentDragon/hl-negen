...
int CController :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
...
void CControllerHeadBall :: HuntThink( void  )
{
	...
	if ((m_hEnemy->Center() - pev->origin).Length() < 64)
	{
		...
		{
			ClearMultiDamage( );
			pEntity->TraceAttack( m_hOwner->pev, gSkillData.controllerDmgZap, pev->velocity, &tr, ID_DMG_CONTROLLER_ENERGYBOLT );
...
void CControllerZapBall::ExplodeTouch( CBaseEntity *pOther )
{
	if (pOther->pev->takedamage)
	{
		...
		ClearMultiDamage( );
		pOther->TraceAttack(pevOwner, gSkillData.controllerDmgBall, pev->velocity.Normalize(), &tr, ID_DMG_CONTROLLER_ENERGYBOLT ); 

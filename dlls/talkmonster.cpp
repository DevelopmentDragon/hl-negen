		...
		if ( pTarget->IsPlayer() )
		{
			if ( pTarget->IsAlive() )
			{
				...
				if (!FBitSet(m_bitsSaid, bit_saidDamageHeavy) && 
					(m_hTargetEnt->pev->health <= m_hTargetEnt->pev->health_max / 8))
				{
					...
					(m_hTargetEnt->pev->health <= m_hTargetEnt->pev->health_max / 4))
				{
					...
					(m_hTargetEnt->pev->health <= m_hTargetEnt->pev->health_max / 2))
				{
...
int CTalkMonster :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iDamageId)
{
	if ( IsAlive() )
	{
...
Schedule_t* CTalkMonster :: GetScheduleOfType ( int Type )
{
	...
	case SCHED_IDLE_STAND:
		{	
			...
			if (!FBitSet(m_bitsSaid, bit_saidWoundLight) && (pev->health <= (pev->health_max * 0.75)))
			{
			...
			else if (!FBitSet(m_bitsSaid, bit_saidWoundHeavy) && (pev->health <= (pev->health_max * 0.5)))
			{

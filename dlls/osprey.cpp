...
/*
int COsprey::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
	if (m_flRotortilt <= -90)
	{
		m_flRotortilt = 0;
...
void COsprey :: DyingThink( void )
{
	...
	else
	{
		...
		*/
		...
		RadiusDamage( pev->origin, pev, pev, 300, CLASS_NONE, ID_DMG_OSPREY_CRASH );
...
void COsprey::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId)
{

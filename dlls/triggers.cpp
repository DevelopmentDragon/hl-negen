...
public:
	...
	int				m_iszMainTrack;
	int				m_iLoopCount;
	int				m_iPosition; // DO NOT USE, DEVELOPERS AND DEBUGGING ONLY!

	CTriggerCDAudio(void);
};

LINK_ENTITY_TO_CLASS( trigger_cdaudio, CTriggerCDAudio );


CTriggerCDAudio::CTriggerCDAudio(void)
{
	m_iszIntroTrack = iStringNull; // No tracks by default
	m_iszMainTrack = iStringNull; // No tracks by default
	m_iLoopCount = 0; // Never loop
	m_iPosition = 0;  // DO NOT USE, DEVELOPERS AND DEBUGGING ONLY!
}

// Sets toucher's friction to m_frictionFraction (1.0 = normal friction)
void CTriggerCDAudio::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "m_iLoopCount"))
	{
		m_iLoopCount = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "m_iszIntroTrack"))
	{
		m_iszIntroTrack = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "m_iszMainTrack"))
	{
		m_iszMainTrack = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "m_iPosition"))
	{
...
TYPEDESCRIPTION	CTriggerCDAudio::m_SaveData[] =
{
	DEFINE_FIELD(CTriggerCDAudio, m_iszIntroTrack, FIELD_STRING),
	DEFINE_FIELD(CTriggerCDAudio, m_iszMainTrack, FIELD_STRING),
	DEFINE_FIELD(CTriggerCDAudio, m_iLoopCount, FIELD_INTEGER),
	DEFINE_FIELD(CTriggerCDAudio, m_iPosition, FIELD_INTEGER),
};

IMPLEMENT_SAVERESTORE(CTriggerCDAudio, CBaseTrigger);
...
void PlayCDTrack( const char* szIntro, const char* szMain, int iLoopCount, int iPosition )
{
	edict_t *pClient;
	...
	// Quad it to make up for string
	char string[256];

	sprintf(string, "cd play %s %s loop %d pos %d\n", szIntro, szMain, iLoopCount, iPosition);
...
void CTriggerCDAudio :: PlayTrackTouch( void )
{
	PlayCDTrack( STRING(m_iszIntroTrack), STRING(m_iszMainTrack), m_iLoopCount, m_iPosition );
...
CTargetCDAudio::CTargetCDAudio(void)
{
	m_iszIntroTrack = iStringNull; // No tracks by default
	m_iszMainTrack = iStringNull; // No tracks by default
	m_iLoopCount = 0; // Never loop
	m_iPosition = 0; // DO NOT USE, DEVELOPERS AND DEBUGGING ONLY!
	m_bDissapearOnUse = TRUE;
}

TYPEDESCRIPTION	CTargetCDAudio::m_SaveData[] =
{
	DEFINE_FIELD(CTriggerCDAudio, m_iszIntroTrack, FIELD_STRING),
	DEFINE_FIELD(CTriggerCDAudio, m_iszMainTrack, FIELD_STRING),
	DEFINE_FIELD(CTargetCDAudio, m_iLoopCount, FIELD_INTEGER),
	DEFINE_FIELD(CTargetCDAudio, m_iPosition, FIELD_INTEGER),
	DEFINE_FIELD(CTargetCDAudio, m_bDissapearOnUse, FIELD_BOOLEAN),
};

IMPLEMENT_SAVERESTORE(CTargetCDAudio, CPointEntity);
...
void CTargetCDAudio :: KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "m_bDissapearOnUse"))
	{
		m_bDissapearOnUse = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}

...
//=========================================================
int CHornet :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
	// filter these bits a little.
	//iDamageId &= ~ ( DMG_ALWAYSGIB );
	//iDamageId |= DMG_NEVERGIB;
...
void CHornet :: Spawn( void )
{
	...
	else
	{
		...
		pev->dmg = 8; // gSkillData.monDmgHornet;
	}
...
void CHornet::DieTouch ( CBaseEntity *pOther )
{
	...
	{// do the damage
		...
		pOther->TakeDamage( pev, VARS( pev->owner ), pev->dmg, ID_DMG_HIVEHAND_HIT_NORMAL );
	}

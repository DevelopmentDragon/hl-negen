...
void CBreakable::BreakTouch( CBaseEntity *pOther )
{
	...
	{// can be broken when run into 
		...
		{
			SetTouch( NULL );
			TakeDamage(pevToucher, pevToucher, flDamage, ID_DMG_GENERIC);

			// do a little damage to player if we broke glass or computer
			pOther->TakeDamage( pev, pev, flDamage/4, ID_DMG_FUNC_BREAK_GLASS );
		}
	}
...
void CBreakable::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId )
{
	...
	CBaseDelay::TraceAttack( pevAttacker, flDamage, vecDir, ptr, iDamageId );
}

//=========================================================
// Special takedamage for func_breakable. Allows us to make
// exceptions that are breakable-specific
// iDamageId indicates the type of damage sustained ie: DMG_CRUSH
//=========================================================
int CBreakable :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iDamageId )
{
	Vector	vecTemp;
	...
	if ( pevAttacker == pevInflictor )	
	{
		...
		// NPCs should also be able to do the same as for sensitivity to crowbar, new damage system covers that
	}
	else
	...
	// Double Crowbar damage and 10% damage? Ridiculous, re-implement this
...
int	CBreakable :: DamageDecal( int iDamageId )
{
	if ( m_Material == matGlass  )
	...
	return CBaseEntity::DamageDecal( iDamageId );
}
...
//	virtual void	SetActivator( CBaseEntity *pActivator ) { m_pPusher = pActivator; }
	...
	virtual int TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iDamageId );
...
int CPushable::TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iDamageId )
{
	if ( pev->spawnflags & SF_PUSH_BREAKABLE )
		return CBreakable::TakeDamage( pevInflictor, pevAttacker, flDamage, iDamageId );

...
void CBeam::BeamDamage( TraceResult *ptr )
{
	...
	if ( ptr->flFraction != 1.0 && ptr->pHit != NULL )
	{
		...
		{
			ClearMultiDamage();
			pHit->TraceAttack( pev, pev->dmg * (gpGlobals->time - pev->dmgtime), (ptr->vecEndPos - pev->origin).Normalize(), ptr, ID_DMG_ENV_BEAM_BURN );
...
void CItemSoda::CanTouch ( CBaseEntity *pOther )
{
	...
	pOther->TakeHealth( 1, ID_HEAL_SODACAN );// a bit of health.

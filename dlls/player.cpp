...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] = 
{
	...
	DEFINE_FIELD( CBasePlayer, m_flNextWeaponIdle, FIELD_TIME ),
...
void LinkUserMessages( void )
{
	...
	gmsgAutoAim = REG_USER_MSG("AutoAim", -1);
}
...
// override takehealth
// iDamageId indicates type of damage healed. 

int CBasePlayer :: TakeHealth( float flHealth, int iDamageId )
{
	return CBaseMonster :: TakeHealth (flHealth, iDamageId);
...
void CBasePlayer :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId)
{
	if ( pev->takedamage )
	{
		...
		case HITGROUP_STOMACH:
			flDamage *= gSkillData.dmgAdjustStomach;
			break;
...
/*
	Take some damage.  
	NOTE: each call to TakeDamage with iDamageId set to a time-based damage
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
	// have suit diagnose the problem - ie: report damage type
	int bitsDamage = iDamageId;
	int ffound = TRUE;
	int fmajor;
	int fcritical;
	int fTookDamage;
	...
	// if ( (iDamageId & DMG_BLAST) && g_pGameRules->IsMultiplayer())
	// {
	...
	// Armor. 
	if (pev->armor /* && !(iDamageId & (DMG_FALL | DMG_DROWN)) */ ) // armor doesn't protect against fall or drown damage!
	{
	...
	fTookDamage = CBaseMonster::TakeDamage(pevInflictor, pevAttacker, (int)flDamage, iDamageId);
		...
		for ( i = 0 ; i < MAX_AMMO_SLOTS ; i++ )
		{
			if ( m_rgAmmo[ i ] > 0 )
			{
				...
				case GR_PLR_DROP_AMMO_ALL:
					// Disabled until we can figure how the hell to manage this
					//iPackAmmo[ iPA++ ] = i;
					break;

				case GR_PLR_DROP_AMMO_ACTIVE:
					// Disabled until we can figure how the hell to manage this
					//if ( m_pActiveItem && i == m_pActiveItem->GetPrimaryAmmoIndex() ) 
					//{
					//	// this is the primary ammo type for the active weapon
					//	iPackAmmo[ iPA++ ] = i;
					//}
					//else if ( m_pActiveItem && i == m_pActiveItem->GetSecondaryAmmoIndex() ) 
					//{
...
void CBasePlayer::RemoveAllItems( BOOL removeSuit )
{
	...
	if (removeSuit)
		...
		pev->weapons &= ~(1 << 32); // WEAPON_ALLWEAPONS;
	...
	if (tr.flFraction != 1.0)
	{
		if ( !FNullEnt( tr.pHit ) )
		{
			...
			if (pEntity->Classify() == CLASS_PLAYER )
			{
				...
				{
					newSBarState[ SBAR_ID_TARGETHEALTH ] = 100 * (pEntity->pev->health / pEntity->pev->health_max);
					newSBarState[ SBAR_ID_TARGETARMOR ] = pEntity->pev->armor; //No need to get it % based since 100 it's the max.
				}
...
// fallpain sound, and damage will be inflicted based on how far the player fell
	...
	if ( (FBitSet(pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		{// after this point, we start doing damage
			...
			if ( flFallDamage > 0 )
			{
				TakeDamage(VARS(eoNullEntity), VARS(eoNullEntity), flFallDamage, 0 ); // DMG_FALL
				pev->punchangle.x = 0;
			}
		}
		...
		if ( m_rgpPlayerItems[ i ] )
		{
			...
			while ( pPlayerItem )
			{
				...
				if ( gun && gun->UseDecrement() )
				{
					...
					gun->m_flNextTertiaryAttack		= max(gun->m_flNextTertiaryAttack - gpGlobals->frametime, -0.001);
					gun->m_flNextQuaternaryAttack =	max(gun->m_flNextQuaternaryAttack - gpGlobals->frametime, -0.001);
					gun->m_flNextQuinaryAttack =	max(gun->m_flNextQuinaryAttack - gpGlobals->frametime, -0.001);
					gun->m_flNextReload				= max(gun->m_flNextReload - gpGlobals->frametime, -0.001);

					if ( gun->m_flNextWeaponIdle != 1000 )
					{
						gun->m_flNextWeaponIdle		= max( gun->m_flNextWeaponIdle - gpGlobals->frametime, -0.001 );
					}
		...
		if ( !FNullEnt( pSpot ) )
		{
			...
			while ( (ent = UTIL_FindEntityInSphere( ent, pSpot->pev->origin, 128 )) != NULL )
			{
				...
				if ( ent->IsPlayer() && !(ent->edict() == player) )
					ent->TakeDamage( VARS(INDEXENT(0)), VARS(INDEXENT(0)), 300, ID_DMG_GENERIC );
			}
			goto ReturnSpot;
		}
	}
...
void CBasePlayer::Spawn( void )
{
	...
	pev->armmodel =		MAKE_STRING("models/arms/hev.mdl");
	pev->armmodelbody = ARM_LEFT_LONG + ARM_RIGHT_LONG;
	pev->armmodelskin = 0;
...
void CBasePlayer::ChangeUpgrade(int iUpgrade)
{
	if (m_pActiveItem)
	{
		m_pActiveItem->m_fWeaponUpgrades = iUpgrade;
		m_pActiveItem->Deploy();
	}
}

void CBasePlayer::ChangeWeaponLook(int iWeaponLook)
{
	if (m_pActiveItem)
	{
		m_pActiveItem->m_iWeaponLooks = iWeaponLook;
		m_pActiveItem->Deploy();
	}
}
...
int CBasePlayer::RemovePlayerItemAlt(CBasePlayerItem* pItem)
{
	if (m_pActiveItem == pItem)
	{
		ResetAutoaim();
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	if (pev->armor != m_iClientBattery)
	{
		m_iClientBattery = pev->armor;

		ASSERT( gmsgBattery > 0 );
		// send "health" update message
		MESSAGE_BEGIN( MSG_ONE, gmsgBattery, NULL, pev );
			WRITE_SHORT( (int)pev->armor);
		MESSAGE_END();
	}
...
{
	TakeDamage ( pevBarnacle, pevBarnacle, pev->health + pev->armor + pev->shield, ID_DMG_BARNACLE_BITE );
}
...
Vector CBasePlayer :: GetAutoaimVector( float flDelta )
{
	...
	*/
	...
	// Don't send across network if sv_aim is 0, thanks Sam for the the suggested fix
...
void CBasePlayer::DropPlayerItem ( char *pszItemName )
{
	...
	for ( i = 0 ; i < MAX_ITEM_TYPES ; i++ )
	{
		...
		if ( pWeapon )
		{
			...
			if ( iAmmoIndex != -1 )
			{
				// Disabled until we can figure out how to manage this
...
{
	if (!pWeapon->CanDeploy() || m_flNextDeploy > 0.0f)
	{
		return FALSE;
	}
	...
	if (m_pActiveItem)
	{
		m_flNextDeploy = m_pActiveItem->GetDeployTime();
...
BOOL CBasePlayer::SwitchWeaponAlt(CBasePlayerItem* pWeapon)
{
	if (!pWeapon->CanDeploy() || m_flNextDeploy > 0.0f)
	{
		return FALSE;
...
void CBasePlayer::ChangeArmModel(const char* szArmName, int bodygroup, int skingroup)
{
	if (!FStrEq(szArmName, "-1"))
	{
		// Check if the arm name is "0", if so, set everything to 0 and return
		if (FStrEq(szArmName, "0"))
		{
			pev->armmodel = iStringNull;
			pev->armmodelbody = 0;
			pev->armmodelskin = 0;
			return;
		}

		// Acquire name and path of file and check if it exists, return if not and don't do anything
		char armName[128];
		sprintf(armName, "models/arms/%s.mdl", szArmName);

		// If model not found, don't load it
		if (!MODEL_INDEX(armName))
		{
			PRECACHE_MODEL(armName);
		}

		ALERT(at_console, "Arm model was %s with string index %d, body %d and skin %d\n", STRING(pev->armmodel), pev->armmodel, pev->armmodelbody, pev->armmodelskin);

		pev->armmodel = ALLOC_STRING(armName);

		// Set the bodygroup
		if (bodygroup > -1)
			pev->armmodelbody = bodygroup;

		if (skingroup > -1)
			pev->armmodelskin = skingroup;

		ALERT(at_console, "Arm model set to %s with string index %d, body %d and skin %d\n", STRING(pev->armmodel), pev->armmodel, pev->armmodelbody, pev->armmodelskin);
	}
}
...
// Modified from Fograin92's deploy logic, albeit with a few qol
void CBasePlayer::DeployLogic(void)
{
	if (m_flNextDeploy > 0)
	{
		m_flNextDeploy -= gpGlobals->frametime;
...
void CBasePlayer::SendWeaponRecoilTest(int iAmount)
{

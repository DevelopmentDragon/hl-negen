...
void CSquidSpit :: Touch ( CBaseEntity *pOther )
{
	...
	{
		pOther->TakeDamage ( pev, pev, gSkillData.bullsquidDmgSpit, ID_DMG_BULLSQUID_SPIT );
	}
...
//=========================================================
int CBullsquid :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
	float flDist;
	Vector vecApex;
...
void CBullsquid :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{
		...
		{
			// SOUND HERE!
			CBaseEntity *pHurt = CheckTraceHullAttack( 70, gSkillData.bullsquidDmgBite, ID_DMG_BULLSQUID_BITE );
		...
		{
			CBaseEntity *pHurt = CheckTraceHullAttack( 70, gSkillData.bullsquidDmgWhip, ID_DMG_BULLSQUID_WHIP );
			if ( pHurt ) 
			{

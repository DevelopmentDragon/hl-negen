...
void CTentacle :: HitTouch( CBaseEntity *pOther )
{
	...
	if (tr.iHitgroup >= 3)
	{
		pOther->TakeDamage( pev, pev, m_iHitDmg, ID_DMG_TENTACLE_HEAD_TOUCH );
		// ALERT( at_console, "wack %3d : ", m_iHitDmg );
	}
	else if (tr.iHitgroup != 0)
	{
		pOther->TakeDamage( pev, pev, 20, ID_DMG_TENTACLE_BODY_TOUCH);
...
int CTentacle::TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iDamageId )
{

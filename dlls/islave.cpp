...
void CISlave :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		{
			// SOUND HERE!
			CBaseEntity *pHurt = CheckTraceHullAttack( 70, gSkillData.slaveDmgClaw, ID_DMG_ISLAVE_CLAWSWIPE );
			if ( pHurt )
			{
		...
		{
			CBaseEntity *pHurt = CheckTraceHullAttack( 70, gSkillData.slaveDmgClawrake, ID_DMG_ISLAVE_CLAWRAKE );
			if ( pHurt )
			{
				if ( pHurt->pev->flags & (FL_MONSTER|FL_CLIENT) )
				{
...
void CISlave::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId)
{
	//if (iDamageId & DMG_SHOCK)
	//	return;
...
void CISlave :: ArmBeam( int side )
{
	...
	DecalGunshot( &tr, BULLET_GENERIC );
...
void CISlave :: ZapBeam( int side )
{
	...
	{
		pEntity->TraceAttack( pev, gSkillData.slaveDmgZap, vecAim, &tr, ID_DMG_ISLAVE_ZAP );
	}

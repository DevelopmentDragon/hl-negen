...
void CStomp::Think( void )
{
	...
	if ( tr.pHit && tr.pHit != pev->owner )
	{
		...
		if ( pEntity )
			pEntity->TakeDamage( pev, pevOwner, gSkillData.gargantuaDmgStomp, ID_DMG_GARGANTUA_STOMP );
	}
...
public:
	...
	void TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId );
	...
	void FlameDamage( Vector vecStart, Vector vecEnd, entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iClassIgnore, int iDamageId );
...
void CGargantua :: FlameUpdate( void )
{
	...
	for ( i = 0; i < 2; i++ )
	{
		if ( m_pFlame[i] )
		{
			...
			FlameDamage( vecStart, trace.vecEndPos, pev, pev, gSkillData.gargantuaDmgFire, CLASS_ALIEN_MONSTER, ID_DMG_GARGANTUA_FLAMES );
...
void CGargantua :: FlameDamage( Vector vecStart, Vector vecEnd, entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iClassIgnore, int iDamageId )
{
...
void CGargantua::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId )
{
	ALERT( at_aiconsole, "CGargantua::TraceAttack\n");

	if ( !IsAlive() )
	{
		CBaseMonster::TraceAttack( pevAttacker, flDamage, vecDir, ptr, iDamageId );
		return;
	}

	// UNDONE: Hit group specific damage?
	if ( /*iDamageId & (GARG_DAMAGE | DMG_BLAST)*/ 0 )
	{
...
int CGargantua::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
	ALERT( at_aiconsole, "CGargantua::TakeDamage\n");

	if ( IsAlive() )
	{
		//if ( !(iDamageId & GARG_DAMAGE) )
		//	flDamage *= 0.01;
		
		//if ( iDamageId & DMG_BLAST )
		//	SetConditions( bits_COND_LIGHT_DAMAGE );
	}

	return CBaseMonster::TakeDamage( pevInflictor, pevAttacker, flDamage, iDamageId );
}
...
void CGargantua::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case GARG_AE_SLASH_LEFT:
		{
			// HACKHACK!!!
			CBaseEntity *pHurt = GargantuaCheckTraceHullAttack( GARG_ATTACKDIST + 10.0, gSkillData.gargantuaDmgSlash, ID_DMG_GARGANTUA_SMASH );
			if (pHurt)
			{

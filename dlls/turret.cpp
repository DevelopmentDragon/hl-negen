...
public:
	...
	virtual void TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId);
	...
	*/
	...
	if (m_fBeserk)
	{
		if (RANDOM_LONG(0,9) == 0)
		{
			...
			TakeDamage(pev,pev,1, ID_DMG_GENERIC); // don't beserk forever
			return;
		}
	} 
...
{
	FireBullets( 1, vecSrc, vecDirToEnemy, TURRET_SPREAD, TURRET_RANGE, BULLET_127MM, 1 );
...
{
	FireBullets( 1, vecSrc, vecDirToEnemy, TURRET_SPREAD, TURRET_RANGE, BULLET_9MM, 1 );
...
void CBaseTurret :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId)
{
	if ( ptr->iHitgroup == 10 )
	{
		// hit armor
...
// take damage. iDamageId indicates type of damage sustained, ie: DMG_BULLET

int CBaseTurret::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId)
{
	if ( !pev->takedamage )
		return 0;
...
{
	FireBullets( 1, vecSrc, vecDirToEnemy, TURRET_SPREAD, TURRET_RANGE, BULLET_9MMH, 1 );
...
int CSentry::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId)
{
	if ( !pev->takedamage )
		return 0;

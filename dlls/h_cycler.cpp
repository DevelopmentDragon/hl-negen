...
int CCycler :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iDamageId )
{
	if (m_animate)
	{
		pev->sequence++;
...
int	CCyclerSprite::TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iDamageId )
{
	if ( m_maxFrame > 1.0 )
	{
		Animate( 1.0 );
...
// DISABLED: Until I figure out whether this has been used in the past or not, we're not touching it.

...
void CApache :: DyingThink( void )
{
	...
	else
	{
		...
		*/
		...
		RadiusDamage( pev->origin, pev, pev, 300, CLASS_NONE, ID_DMG_APACHE_CRASH );
...
BOOL CApache :: FireGun( )
{
	...
	{
#if 1
		FireBullets( 1, posGun, vecGun, VECTOR_CONE_4DEGREES, 8192, BULLET_127MM, 1 );
...
int CApache :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iDamageId )
{
	...
	/*
	if ( (iDamageId & DMG_BULLET) && flDamage > 50)
	{
	...
	*/
	...
	// ALERT( at_console, "%.0f\n", flDamage );
	return CBaseEntity::TakeDamage(  pevInflictor, pevAttacker, flDamage, iDamageId );
}



void CApache::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId)
{
	...
	else
	{
		...
		// AddMultiDamage( pevAttacker, this, flDamage / 2.0, iDamageId );

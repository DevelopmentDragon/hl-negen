...
BOOL CHGrunt :: CheckRangeAttack2 ( float flDot, float flDist )
{
	...
	else
	{
		...
		// estimate position
		if (HasConditions( bits_COND_SEE_ENEMY))
			vecTarget = vecTarget + ((vecTarget - pev->origin).Length() / 300) * m_hEnemy->pev->velocity; // gSkillData.hgruntGrenadeSpeed
	}
	...
	{
		Vector vecToss = VecCheckThrow( pev, GetGunPosition(), vecTarget, 300 , 0.5 ); //gSkillData.hgruntGrenadeSpeed
...
//=========================================================
void CHGrunt :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId)
{
	// check for helmet shot
	if (ptr->iHitgroup == 11)
	{
		// make sure we're wearing one
		if (GetBodygroup( 1 ) == HEAD_GRUNT && IsBullet(0) || IsMeleeBullet(0))
		{
			// absorb damage
			flDamage -= 20;
			if (flDamage <= 0)
	...
	CSquadMonster::TraceAttack( pevAttacker, flDamage, vecDir, ptr, iDamageId );
}
...
//=========================================================
int CHGrunt :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
...
void CHGrunt :: Shoot ( void )
{
	...
	FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_10DEGREES, 2048, BULLET_9MMH ); // shoot +-5 degrees
	...
	EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/mp5/fire.wav", 1, ATTN_NORM);
...
void CHGrunt :: Shotgun ( void )
{
	...
	FireBullets(8, vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES, 2048, BULLET_12G, 0 ); // shoot +-7.5 degrees
	...
	switch( pEvent->event )
	{
		...
		case HGRUNT_AE_KICK:
		{
			...
			if ( pHurt )
			{
				...
				pHurt->TakeDamage( pev, pev, gSkillData.hgruntDmgKick, ID_DMG_HGRUNT_KICK );
			}
		}
		break;

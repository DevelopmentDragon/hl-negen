...
int CLeech::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
	...
	case LEECH_AE_ATTACK:
		...
		if ( pEnemy != NULL )
		{
			...
			if ( DotProduct(dir, face) > 0.9 )		// Only take damage if the leech is facing the prey
				pEnemy->TakeDamage( pev, pev, gSkillData.leechDmgBite, ID_DMG_LEECH_BITE );
		}
		m_stateTime -= 2;
		break;

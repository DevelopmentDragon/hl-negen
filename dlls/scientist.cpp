...
int CScientist :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId)
{
	...
	return CTalkMonster::TakeDamage(pevInflictor, pevAttacker, flDamage, iDamageId);
}
...
{ 
	if ( (m_healTime > gpGlobals->time) || (m_hTargetEnt == NULL) || (m_hTargetEnt->pev->health > (m_hTargetEnt->pev->health_max * 0.5)) )
		return FALSE;
...
void CScientist::Heal( void )
{
	...
	m_hTargetEnt->TakeHealth( gSkillData.scientistHeal, ID_HEAL_GENERIC );

...
int CNihilanth :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iDamageId )
{
...
void CNihilanth::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId)
{
	if (m_irritation == 3)
		m_irritation = 2;
...
void CNihilanthHVR :: ZapThink( void  )
{
	...
	if ((m_hEnemy->Center() - pev->origin).Length() < 256)
	{
		...
		{
			ClearMultiDamage( );
			pEntity->TraceAttack( pev, gSkillData.nihilanthDmgZap, pev->velocity, &tr, ID_DMG_NIHILANTH_BALLZAP );
...
void CNihilanthHVR::ZapTouch( CBaseEntity *pOther )
{
	...
	RadiusDamage( pev, pev, 50, CLASS_NONE, ID_DMG_NIHILANTH_BALLZAP );

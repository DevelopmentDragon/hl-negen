...
void CBigMomma :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{
		...
		case BIG_AE_MELEE_ATTACK1:
		{
			...
			if ( pHurt )
			{
				pHurt->TakeDamage( pev, pev, gSkillData.bigmommaDmgSlash, ID_DMG_GONARCH_CLAW );
...
void CBigMomma :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int iDamageId )
{
	if ( ptr->iHitgroup != 1 )
	{
		// didn't hit the sack?
...
int CBigMomma :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
	// Don't take any acid damage -- BigMomma's mortar is acid
	if ( iDamageId == ID_DMG_GONARCH_MORTAR )
...
void CBigMomma::NodeReach( void )
{
	...
	if ( pTarget->pev->health )
		pev->health_max = pev->health = pTarget->pev->health * gSkillData.bigmommaHealthFactor;
...
void CBMortar::Touch( CBaseEntity *pOther )
{
	...
	RadiusDamage( pev->origin, pev, pevOwner, gSkillData.bigmommaDmgBlast, gSkillData.bigmommaRadiusBlast, CLASS_NONE, ID_DMG_GONARCH_MORTAR );
	UTIL_Remove( this );
}

...
void CFuncPlat :: Blocked( CBaseEntity *pOther )
{
	...
	pOther->TakeDamage(pev, pev, 1, ID_DMG_FUNC_PLAT_BLOCK);
...

{
	...
	pOther->TakeDamage(pev, pev, pev->dmg, ID_DMG_FUNC_TRAIN_BLOCK);
}
...
void CFuncTrackTrain :: Blocked( CBaseEntity *pOther )
{
	...
	pOther->TakeDamage(pev, pev, pev->dmg, ID_DMG_FUNC_TRACKTRAIN_BLOCK);
}
...
void CFuncTrackTrain :: StopSound( void )
{
	...
	if (m_soundPlaying && pev->noise)
	{
		...
		PLAYBACK_EVENT_FULL( FEV_RELIABLE | FEV_UPDATE, edict(), m_usAdjustPitch, 0.0, 
			(float *)&g_vecZero, (float *)&g_vecZero, 0.0, 0.0, us_encode, 0, 1, 0, (float*)&g_vecZero, (float*)&g_vecZero, 0, 0.0f, (byte)0);
...
*/
		...
		PLAYBACK_EVENT_FULL( FEV_RELIABLE | FEV_UPDATE, edict(), m_usAdjustPitch, 0.0, 
			(float *)&g_vecZero, (float *)&g_vecZero, 0.0, 0.0, us_encode, 0, 0, 0, (float*)&g_vecZero, (float*)&g_vecZero, 0, 0.0f, (byte)0);
	}
}
...
// pev->health_max is the amount to reset to each time it starts
...
int	CGunTarget::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iDamageId )
{
	if ( pev->health > 0 )
	{
		pev->health -= flDamage;

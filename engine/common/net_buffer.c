...
void MSG_WriteBitFlag(sizebuf_t* sb, flag curData, int numbits)
{
...
void MSG_WriteFlag(sizebuf_t* sb, flag val)
{
	MSG_WriteBitFlag(sb, val, sizeof(flag) << 3);
}
...
void MSG_WriteFloatAlt(sizebuf_t* sb, float val)
{
	MSG_WriteSBitLong(sb, &val, sizeof(float) << 3);
}
...
flag MSG_ReadBitFlag(sizebuf_t* sb, int numbits)
{
	int	idword1;
	uint	dword1, ret;
	...
	idword1 = sb->iCurBit >> 5;
	dword1 = ((uint*)sb->pData)[idword1];
	dword1 >>= (sb->iCurBit & 63);	// get the bits we're interested in.
...
flag MSG_ReadFlag(sizebuf_t* sb)
{
	return MSG_ReadBitFlag(sb, sizeof(flag) << 3);
}
...
float MSG_ReadFloatAlt (sizebuf_t* sb)
{
	return MSG_ReadSBitLong(sb, sizeof(float) << 3);
}

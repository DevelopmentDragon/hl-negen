...
{ WPDT_DEF(m_flNextTertiaryAttack)			},
{ WPDT_DEF(m_flNextQuaternaryAttack)  },
{ WPDT_DEF(m_flNextQuinaryAttack)  },
{ WPDT_DEF(m_flNextReload) 	},
{ WPDT_DEF(m_flNextWeaponIdle)			},
{ WPDT_DEF(m_flNextRecoil)			},
{ WPDT_DEF(m_flNextCharge)			},
{ WPDT_DEF(m_flNextPump)	},
{ WPDT_DEF(m_flNextSwitch)			},
{ WPDT_DEF(m_fWeaponStatusFlags)			},
{ WPDT_DEF( m_iId )			},
{ WPDT_DEF( m_iClip )		},
...
{ CLDT_DEF( view_ofs[2] )	},
{ CLDT_DEF(health)	},
{ CLDT_DEF(health_type) },
{ CLDT_DEF(health_max)	},
{ CLDT_DEF(health_over_max)	},
{ CLDT_DEF(health_temp)	},
{ CLDT_DEF(health_temp_max)	},
{ CLDT_DEF(armor)	},
{ CLDT_DEF(armor_type)	},
{ CLDT_DEF(armor_max)	},
{ CLDT_DEF(armor_over_max)	},
{ CLDT_DEF(armor_temp)	},
{ CLDT_DEF(armor_temp_max)	},
{ CLDT_DEF(shield)	},
{ CLDT_DEF(shield_type)	},
{ CLDT_DEF(shield_max)	},
{ CLDT_DEF(shield_over_max)	},
{ CLDT_DEF(shield_temp)	},
{ CLDT_DEF(shield_temp_max)	},
{ CLDT_DEF( bInDuck )	},
{ CLDT_DEF( weapons )	},
...
{ CLDT_DEF( fov )		},
{ CLDT_DEF( weaponanim )	},
{ CLDT_DEF(viewmodelbody)	},
{ CLDT_DEF(armmodelbody)	},
{ CLDT_DEF(playmodelbody)	},
{ CLDT_DEF(viewmodelskin)	},
{ CLDT_DEF(armmodelskin)	},
{ CLDT_DEF(playmodelskin)	},
{ CLDT_DEF(armmodel)	},
{ CLDT_DEF(savedvanim) },
{ CLDT_DEF(savedvframerate) },
{ CLDT_DEF(savedvframe) },
{ CLDT_DEF( m_iId )		},
...
{ ENTS_DEF( framerate )	},
{ ENTS_DEF(body)		},
{ ENTS_DEF(viewmodelbody)	},
{ ENTS_DEF(armmodelbody)	},
{ ENTS_DEF(playmodelbody)	},
{ ENTS_DEF(skin)		},
{ ENTS_DEF(viewmodelskin)	},
{ ENTS_DEF(armmodelskin)	},
{ ENTS_DEF(playmodelskin)	},
{ ENTS_DEF(savedvanim) },
{ ENTS_DEF(savedvframerate) },
{ ENTS_DEF(savedvframe) },
...
{ ENTS_DEF( health )	},
{ ENTS_DEF(health_type ) },
{ ENTS_DEF(health_max )	},
{ ENTS_DEF(health_over_max )	},
{ ENTS_DEF(health_temp )	},
{ ENTS_DEF(health_temp_max )	},
{ ENTS_DEF(armor )	},
{ ENTS_DEF(armor_type )	},
{ ENTS_DEF(armor_max )	},
{ ENTS_DEF(armor_over_max )	},
{ ENTS_DEF(armor_temp )	},
{ ENTS_DEF(armor_temp_max )	},
{ ENTS_DEF(shield )	},
{ ENTS_DEF(shield_type )	},
{ ENTS_DEF(shield_max )	},
{ ENTS_DEF(shield_over_max )	},
{ ENTS_DEF(shield_temp )	},
{ ENTS_DEF(shield_temp_max )	},
{ ENTS_DEF( spectator )	},
{ ENTS_DEF(armmodel) },
...
qboolean Delta_ParseField( char **delta_script, const delta_field_t *pInfo, delta_t *pField, qboolean bPost )
{
	...
	*delta_script = COM_ParseFile( *delta_script, token );
	...
	while(( *delta_script = COM_ParseFile( *delta_script, token )) != NULL )
	{
		...
		else if (!Q_strcmp(token, "DT_FLAG"))
			pField->flags |= DT_FLAG;
	}
...
qboolean Delta_CompareField( delta_t *pField, void *from, void *to, float timebase )
{
	...
	else if (pField->flags & DT_FLAG)
	{
		fromF = *(flag*)((byte*)from + pField->offset);
		toF = *(flag*)((byte*)to + pField->offset);
...
qboolean Delta_WriteField( sizebuf_t *msg, delta_t *pField, void *from, void *to, float timebase )
{
	...
	{
		fValue = *(flag*)((byte*)to + pField->offset);
		if (pField->multiplier != 1.0f) fValue *= pField->multiplier;
		MSG_WriteBitFlag(msg, fValue, pField->bits);
	}
...
qboolean Delta_ReadField( sizebuf_t *msg, delta_t *pField, void *from, void *to, float timebase )
{
	...
	else if (pField->flags & DT_FLAG)
	{
		if (bChanged)
		{
			fValue = MSG_ReadBitFlag(msg, pField->bits);
			if (pField->multiplier != 1.0f) fValue /= pField->multiplier;
		}
		else
		{
			fValue = *(flag*)((byte*)from + pField->offset);
		}
		*(flag*)((byte*)to + pField->offset) = fValue;
	}
	else if( pField->flags & DT_FLOAT )
	{
		if( bChanged )

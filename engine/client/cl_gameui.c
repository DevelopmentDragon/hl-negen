...
/*
====================
PIC_DrawGenericScale

draw hudsprite routine
====================
*/
static void PIC_DrawGenericScale(float x, float y, float width, float height, const wrect_t* prc, float xscale, float yscale, float selfangle, float angle, int xpivot, int ypivot, int selfxpivot, int selfypivot)
{
	float	s1, s2, t1, t2;
	int	w, h;
	...
	PicAdjustScale(&width, &height, xscale, yscale);
	PicAdjustSize(&x, &y, &width, &height);
	R_DrawStretchPicRotate(x, y, width, height, s1, t1, s2, t2, gameui.ds.gl_texturenum, xscale, yscale, selfangle, angle, xpivot, ypivot, selfxpivot, selfypivot);
	pglColor4ub(255, 255, 255, 255);
}
...
/*
=========
pfnPIC_DrawAdditiveAlt

=========
*/
void pfnPIC_DrawAdditiveAlt(int x, int y, int width, int height, const wrect_t* prc, float xscale, float yscale, float selfangle, float angle, int xpivot, int ypivot, int selfxpivot, int selfypivot)
{
	GL_SetRenderMode(kRenderTransAdd);
	PIC_DrawGenericScale(x, y, width, height, prc, xscale, yscale, selfangle, angle, xpivot, ypivot, selfxpivot, selfypivot);
}
...
=============
*/
static void pfnFillRGBA(int x, int y, int width, int height, int type, float thickness, int r, int g, int b, int a, float xscale, float yscale, float selfangle, float angle, float xpivot, float ypivot)
{
	r = bound( 0, r, 255 );
	g = bound( 0, g, 255 );
	b = bound( 0, b, 255 );
	...
	pglColor4ub( r, g, b, a );
	GL_SetRenderMode( kRenderTransTexture );
	R_DrawStretchPicRotate(x, y, width, height, 0, 0, 1, 1, tr.whiteTexture, xscale, yscale, selfangle, angle, xpivot, ypivot, -1, -1);
	pglColor4ub( 255, 255, 255, 255 );
}
...
=============
*/
static void pfnFillRGBACircle(int x, int y, float width, float height, int numshapes, int type, float thickness, float angle_start, float angle_end, int r, int g, int b, int a, float xscale, float yscale, float selfangle, float angle, float xpivot, float ypivot)
{
	r = bound(0, r, 255);
	g = bound(0, g, 255);
	b = bound(0, b, 255);
	...
	R_DrawStretchPicCircleRotate(x, y, width, height, numshapes, type, thickness, angle_start, angle_end, tr.whiteTexture, xscale, yscale, selfangle, angle, xpivot, ypivot);
	pglColor4ub(255, 255, 255, 255);
}
...
=========
*/
static void pfnStartBackgroundTrack( const char *introTrack, const char *mainTrack, long position, int loopCount, int onLoop, int restored )
{
	S_StartBackgroundTrack( introTrack, mainTrack, position, loopCount, onLoop, restored, true );
}

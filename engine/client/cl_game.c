...
====================
*/
static void SPR_AdjustScale(float* w, float* h, float xscale, float yscale)
{
	if (!xscale && !yscale && !w && !h) return;
...
void PicAdjustScale(float* w, float* h, float xscale, float yscale)
{
	if (!xscale && !yscale && !w && !h) return;
...
/*
====================
SPR_DrawGenericScale

draw hudsprite routine
====================
*/
static void SPR_DrawGenericScale(int frame, float x, float y, float width, float height, const wrect_t* prc, float xscale, float yscale, float selfangle, float angle, float xpivot, float ypivot, float selfxpivot, float selfypivot)
{
	float	s1, s2, t1, t2;
	int	texnum;
	...
	// scale for screen sizes
	SPR_AdjustScale(&width, &height, xscale, yscale);
	SPR_AdjustSize(&x, &y, &width, &height);
	texnum = R_GetSpriteTexture(clgame.ds.pSprite, frame);
	pglColor4ubv(clgame.ds.spriteColor);
	pglTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	...
	pglPushMatrix();
	pglPopMatrix();
	R_DrawStretchPicRotate(x, y, width, height, s1, t1, s2, t2, texnum, xscale, yscale, selfangle, angle, xpivot, ypivot, selfxpivot, selfypivot);
;
}
...
// These two functions are purely for auto-aim purposes, I hope to god it works
int CL_GetCrosshairPosX(void)
{
	// any camera on or client is died
	if (cl.local.health <= 0 || cl.viewentity != (cl.playernum + 1))
		return;

	int x = clgame.viewport[0] + (clgame.viewport[2] >> 1);
...
int CL_GetCrosshairPosY(void)
{
	// any camera on or client is died
	if (cl.local.health <= 0 || cl.viewentity != (cl.playernum + 1))
		return;

	int y = clgame.viewport[1] + (clgame.viewport[3] >> 1);
...
=============
*/
const model_t *CL_GetSpritePointer( HSPR hSprite )
{
	model_t	*mod;
...
=========
*/
static void pfnSPR_DrawAdditiveScale(int frame, int x, int y, const wrect_t* prc, float xscale, float yscale, float selfangle, float angle, float xpivot, float ypivot)
{
	pglEnable(GL_BLEND);
	pglBlendFunc(GL_ONE, GL_ONE);

	SPR_DrawGenericScale(frame, x, y, -1, -1, prc, xscale, yscale, selfangle, angle, xpivot, ypivot, -1, -1);
...
=============
*/
void CL_FillRGBA(int x, int y, int width, int height, int type, float thickness, int r, int g, int b, int a, float xscale, float yscale, float selfangle, float angle, float xpivot, float ypivot)
{
	r = bound( 0, r, 255 );
	g = bound( 0, g, 255 );
	b = bound( 0, b, 255 );
	...
	if (type == 1 || type == 2)
		...
		pglTranslatef(x + (width / 2), y + (height / 2), 0.0);
	...
	case 2:
		{
			...
			pglVertex2f(
				x + (xscale * width * cos(0)),
				y + (yscale * height * sin(0))
			);
			if (type == 2)
			{
				pglVertex2f(
					x + (xscale * width * cos(M_PI)),
					y + (yscale * height * sin(M_PI))
				);
			}
			else
			{
				pglVertex2f(x, y); // draw center of circle
			}
			pglEnd();
			pglLineWidth(1.0);
		}
...
=============
*/
void CL_FillRGBACircle(int x, int y, float width, float height, int numshapes, int type, float thickness, float angle_start, float angle_end, int r, int g, int b, int a, float xscale, float yscale, float selfangle, float angle, float xpivot, float ypivot)
{
	// Sanity check, do not draw if we do not recognize circle shape
	if (type < 0 || type > 4)
		return;
	...
	//SPR_AdjustSize((float*)&x, (float*)&y, (float*)&radius, (float*)&radius);
	...
	for (i = 0; i <= adjusted_numshapes; i++) {
		pglVertex2f(
			x + (xscale * width * cos(i * adjusted_twicepi / adjusted_numshapes)),
			y + (yscale * height * sin(i * adjusted_twicepi / adjusted_numshapes))
		);
	}
	pglEnd();
	pglLineWidth(1.0);
...
// Copied from gl_vidint.c, only way I could find that allowed us to gather information about resolution
typedef struct vidmode2_s
{
	const char* desc;
	int		width;
	int		height;
...
static int pfnGetScreenInfo( SCREENINFO *pscrinfo )
{
	...
	// Get HUD Scale
	int hud_scale = Cvar_VariableInteger("hud_scale");

	// Catch stuff you don't want to happen
	if (hud_scale < -1 || hud_scale >= sizeof(vidmode2)) hud_scale = -1;

	if( hud_scale != -1 )
	{
		int hud_scale_width = vidmode2[hud_scale].width;
		int hud_scale_height = vidmode2[hud_scale].height;

		if (hud_scale_width != glState.width || hud_scale_height != glState.height)
		{
			// virtual screen space 640x480
			clgame.scrInfo.iWidth = hud_scale_width;
			clgame.scrInfo.iHeight = hud_scale_height;
	...
	// Always acquire this as is for scaling routines
	clgame.scrInfo.iHeightOrg = glState.height;
	clgame.scrInfo.iWidthOrg = glState.width;
...
=============
*/
static int pfnDrawCharacterAlt(int x, int y, int number, int r, int g, int b, int a, float xscale, float yscale, int xpivot, int ypivot, float angle, float selfangle, int selfxpivot, int selfypivot)
{
	if (!cls.creditsFont.valid)
		return 0;
	...
	pfnPIC_DrawAdditiveAlt(x, y, -1, -1, &cls.creditsFont.fontRc[number], xscale, yscale, selfangle, angle, xpivot, ypivot, selfxpivot, selfypivot); // Turn into new version
	clgame.ds.adjust_size = false;

...
void CL_WeaponAnim( int iAnim, float flFramerate, byte byFrame )
{
	...
	cl.local.weaponstartframe = byFrame;
	cl.local.weaponsequence = iAnim;
	view->curstate.framerate = flFramerate;
...
void CL_ParseUserMessage( sizebuf_t *msg, int svc_num )
{
	...
	if( clgame.msg[i].func )
	{
		...
		if( !Q_stricmp( clgame.msg[i].name, "HudText" ) && !Q_stricmp( GI->gamefolder, "fareast" ))
		{
...
void CL_ParseServerMessage( sizebuf_t *msg, qboolean normal_message )
{
	...
	byte byParam1, byParam2;
	float flParam1, flParam2;
	int		old_background;
	const char	*s;
	...
	while( 1 )
	{
		...
		case svc_finale:
			...
			break;
		case svc_cdtrack:
			S_StartBackgroundTrack(MSG_ReadString(msg), MSG_ReadString(msg), MSG_ReadLong(msg), MSG_ReadLong(msg), MSG_ReadLong(msg), MSG_ReadLong(msg), true );
			break;
		case svc_restore:
			CL_ParseRestore( msg );
			break;
		case svc_cutscene:
			...
			break;
		case svc_weaponanim:
			param1 = MSG_ReadLong( msg );	// iAnim
			flParam1 = MSG_ReadFloat( msg );	// flFramerate
			byParam1 = MSG_ReadByte(msg); // byFrame
			CL_WeaponAnim( param1, flParam1, byParam1 );
			break;
		case svc_bspdecal:

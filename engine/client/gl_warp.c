...
static struct
{
	...
	unsigned int texture[RIPPLES_TEXSIZE];
	int gl_texturenum;
...
void EmitWaterPolys( msurface_t *warp, qboolean reverse )
{
	...
	//if (r_ripple->value) waveHeight = 0;
	...
	for( p = warp->polys; p; p = p->next )
	{
		...
		for( i = 0; i < p->numverts; i++ )
		{
			...
			s = (os + r_turbsin[(int)((ot * 0.125f + cl.time) * TURBSCALE) & 255]) / ((g_ripple.texturescale != 0.0f) ? 1.0f : g_ripple.texturescale);
			s *= ( 1.0f / SUBDIVIDE_SIZE );

			t = (ot + r_turbsin[(int)((os * 0.125f + cl.time) * TURBSCALE) & 255]) / ((g_ripple.texturescale != 0.0f) ? 1.0f : g_ripple.texturescale);
...
void R_UploadRipples(texture_t* image)
{
	...
	pixels = (unsigned long*)glt->original->buffer;

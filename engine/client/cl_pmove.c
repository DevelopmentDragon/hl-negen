...
{
	CL_PlaybackEvent(flags, NULL, eventindex, delay, origin, angles, fparam1, fparam2, iparam1, iparam2, bparam1, bparam2, vparam1, vparam2, aparam1, aparam2, aparam3);
}
...
void CL_PredictMovement( qboolean repredicting )
{
	...
	if( !CL_IsPredicted( ))
	{
		...
		if (!repredicting || !CVAR_TO_BOOL(cl_lw))
		{
			...
			cl.local.armmodel = to->client.armmodel;
		}
		cl.local.repredicting = false;

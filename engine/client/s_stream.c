...
=================
*/
void S_StartBackgroundTrack( const char *introTrack, const char *mainTrack, long position, int loopCount, qboolean onLoop, qboolean restored, qboolean fullpath )
{
	...
	// Do not start anything if either tracks are null
	if( !COM_CheckString( introTrack ) && !COM_CheckString( mainTrack ))
		return;

	// Now we need to set up the track
	int iWhich = 1 * COM_CheckString(introTrack) + 2 * COM_CheckString(mainTrack);

	// If neither are valid, don't bother at all
	if (!(iWhich & (1 | 2)))
		return;
	...
	if (!restored)
	{
		...
		{
			if ((iWhich & (1 | 2)) == 3)
				s_bgTrack.loopCount = loopCount;
			else
				s_bgTrack.loopCount = loopCount - 1;
		}
		else
		{
			if (loopCount <= -1)
			{
				s_bgTrack.loopCount = -1;
			}
			else
			{
				s_bgTrack.loopCount = 1;	
	...
	if (restored)
	{
		// Loop count must be preserved, do not change it
		if (loopCount == 0)
			loopCount++;
		s_bgTrack.loopCount = loopCount;

		// If we're restoring a track
		if (onLoop)
		{
			s_bgTrack.stream = FS_OpenStream(va("%s", mainTrack));
			s_bgTrack.onLoop = true;
		}
		else
		{
			s_bgTrack.stream = FS_OpenStream(va("%s", introTrack));
			s_bgTrack.onLoop = false;
		}
	}
	else
	{
		// In case we're not restoring a track
		if (iWhich & 1 == 1)
		{
			// New code
			s_bgTrack.stream = FS_OpenStream(va("%s", mainTrack));
			if (!s_bgTrack.stream && s_bgTrack.loopCount > 0) s_bgTrack.loopCount--;
...
=================
*/
qboolean S_StreamGetCurrentState( char *currentTrack, char *loopTrack, int *position, int *loopNum, int *onLoop )
{
	if( !s_bgTrack.stream )
	...
	if (loopNum)
		*loopNum = s_bgTrack.loopCount;

	if (onLoop)
		*onLoop = s_bgTrack.onLoop;
...
void S_StreamBackgroundTrack( void )
{
	...
	while( ch->s_rawend < soundtime + ch->max_samples )
	{
		...
		{
			// loop section
			if ((s_bgTrack.loopCount < 0 || s_bgTrack.loopCount > 0))
			{
				if (s_bgTrack.loopCount > 0)
					--s_bgTrack.loopCount;
				...
				if (s_bgTrack.loopName[0])
				{
					...
					if (!s_bgTrack.stream)
					{
						s_bgTrack.stream = FS_OpenStream(s_bgTrack.current);
						s_bgTrack.onLoop = false;
					}
				}
				else

...
void S_Music_f( void )
{
	...
	if( c == 1 )
	{
		// Instruct on usage of command instead of stopping track, adding "stop" command to this
		Con_Printf(S_USAGE "music <musicfile> [loopfile] [loop] [numLoops] [pos] [position] [onloop] [restored]\n");
	}
	else
	{
		// Having this stops the track, don't bother with anything else afterwards
		if (!Q_strcmp(Cmd_Argv(1), "stop"))
		{
			S_StopBackgroundTrack();
			return;
		}
		...
		if (loopArgPos != -1)
		{
			if (loopArgPos < (c - 1) && Q_isdigit(Cmd_Argv(loopArgPos + 1)))
		...
		if (posArgPos != -1)
		{
			if (posArgPos < (c - 1) && Q_isdigit(Cmd_Argv(posArgPos + 1)))
				position = atol(Cmd_Argv(posArgPos + 1));
		}

		//Con_Printf("loopArgPos: %d\n", loopArgPos);
		//Con_Printf("posArgPos: %d\n", posArgPos);
		//Con_Printf("argc: %d\n", c);
		//Con_Printf("restored: %d\n", restored);

		// If it's not loop on the third argument, then it's a second section of the song
		Q_snprintf(intro, sizeof(intro), "%s", Cmd_Argv(1)); // if c is not equal to 1, it's either 2 or higher, always make 1 be the track name
		if (loopArgPos != 2 && posArgPos != 2 && onLoopPos != 2 && restoredPos != 2 && c >= 3)
			Q_snprintf(main, sizeof(main), "%s", Cmd_Argv(2));
		else
			Q_snprintf(main, sizeof(main), "%s", "");

		//Con_Printf("main: %s\n", main);

		S_StartBackgroundTrack(intro, main, position, numLoop, onLoop, restored, true);
	}
}

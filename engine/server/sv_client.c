...
{
	static byte    	msg_buf[0x20200 * 2];	// MAX_INIT_MSG + some space
	...
	if( sv.loadgame )
	{
		...
		MSG_WriteLong(&msg, (long)(ent->v.savedvanim));
		MSG_WriteFloat(&msg, (float)(ent->v.savedvframerate));
		MSG_WriteByte(&msg, (byte)(ent->v.savedvframe));
...
void SV_TSourceEngineQuery( netadr_t from )
{
	...
	if( Q_stricmp( GI->gamedir, "fareast" ))
	{

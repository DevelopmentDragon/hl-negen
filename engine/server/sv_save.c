...
static TYPEDESCRIPTION gSaveClient[] =
{
	...
	DEFINE_FIELD(SAVE_CLIENT, loopNum, FIELD_INTEGER),
	DEFINE_FIELD(SAVE_CLIENT, onLoop, FIELD_INTEGER),
...
static void ParseSaveTables( SAVERESTOREDATA *pSaveData, SAVE_HEADER *pHeader, int updateGlobals )
{
	...
	// START: Changes implemented thanks to Nikita Butorin
	pSaveData->pTable = Mem_Calloc(host.mempool, sizeof(ENTITYTABLE) * pSaveData->tableCount);
	pSaveData->tableCount = pSaveData->tableCount;
	...
	// END: Changes implemented thanks to Nikita Butorin
...
static void SaveClientState( SAVERESTOREDATA *pSaveData, const char *level, int changelevel )
{
	...
	if( !changelevel )
	{
		...
		S_StreamGetCurrentState(header.introTrack, header.mainTrack, &header.trackPosition, &header.loopNum, &header.onLoop);
		Con_Printf("saveMusic: onLoop: %d\n", header.onLoop);
	}
...
static SAVERESTOREDATA *SaveGameState( int changelevel )
{
	...
	header.skyDir_x = sv_skydir_x.value;
	header.skyDir_y = sv_skydir_y.value;
	header.skyDir_z = sv_skydir_z.value;
	header.skyAngle = sv_skyangle.value;
	header.skySpeed = sv_skyspeed.value;

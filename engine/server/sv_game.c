...
void SV_RestartAmbientSounds( void )
{
	...
	// restart soundtrack
	if( S_StreamGetCurrentState( curtrack, looptrack, &position, &numLoop, &onLoop ))
	{
		Con_Printf("Music has been restored!\n");
		SV_StartMusic( curtrack, looptrack, position, numLoop, onLoop, true );
	}
}
...
int SV_BuildSoundMsg( sizebuf_t *msg, edict_t *ent, int chan, const char *sample, int vol, float attn, int flags, int pitch, const vec3_t pos )
{
	...
	if( chan < CHAN_MIN_NORMAL || chan > CHAN_MAX_NORMAL )
	{
		Con_Reportf( S_ERROR "SV_StartSound: channel must be in range 0-7\n" );
		chan = bound(CHAN_MIN_NORMAL, chan, CHAN_MAX_NORMAL);
	}
...
=================
*/
void SV_StartMusic( const char *curtrack, const char *looptrack, long position, int numLoop, int onLoop, int restored )
{
	MSG_BeginServerCmd( &sv.multicast, svc_stufftext );
	if (numLoop < 0 || numLoop > 2)
		MSG_WriteString( &sv.multicast, va( "music \"%s\" \"%s\" loop %d pos %i%s%s\n", curtrack, looptrack, numLoop, position, (onLoop) ? " onloop" : "", (restored) ? " restored" : ""));
	else
		MSG_WriteString(&sv.multicast, va("music \"%s\" \"%s\" pos %i%s%s\n", curtrack, looptrack, position, (onLoop) ? " onloop" : "", (restored) ? " restored" : ""));
...
=============
*/
void pfnWriteFlag(flag fValue)
{
	MSG_WriteFlag(&sv.multicast, fValue);
	...
	float* angles, float fparam1, float fparam2, int iparam1, int iparam2, int bparam1, int bparam2, float* vparam1, float* vparam2, int aparam1, float aparam2, byte aparam3)
{
	...
	if (vparam1 && !VectorIsNull(vparam1))
		VectorCopy(vparam1, args.vparam1);

	if (vparam2 && !VectorIsNull(vparam2))
		VectorCopy(vparam2, args.vparam2);
	...
	args.aparam1 = aparam1;
	args.aparam2 = aparam2;
	args.aparam3 = aparam3;

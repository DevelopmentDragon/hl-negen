...
// Wrapper function needed for default values
void EV_WeaponAnim(int sequence, float flFramerate = 1.0f, byte byFrame = 0.0f);

// Defined version placed here
void EV_WeaponAnim(int sequence, float flFramerate, byte byFrame )
{
	gEngfuncs.pEventAPI->EV_WeaponAnimation(sequence, flFramerate, byFrame);
}
...
void EV_FireGlock2( event_args_t *args )
{
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_9MM, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
void EV_FireShotGunDouble( event_args_t *args )
{
	...
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 8, vecSrc, vecAiming, 2048, BULLET_12G, 0, &tracerCount[idx-1], 0.17365, 0.04362 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 12, vecSrc, vecAiming, 2048, BULLET_12G, 0, &tracerCount[idx-1], 0.08716, 0.08716 );
	}
}
...
void EV_FireShotGunSingle( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 4, vecSrc, vecAiming, 2048, BULLET_12G, 0, &tracerCount[idx-1], 0.08716, 0.04362 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 6, vecSrc, vecAiming, 2048, BULLET_12G, 0, &tracerCount[idx-1], 0.08716, 0.08716 );
	}
}
//======================
//	   SHOTGUN END
...
void EV_FireMP5(event_args_t* args)
{
	...
	shell = gEngfuncs.pEventAPI->EV_FindModelIndex("models/shells/9mmh.mdl");// brass shell
	...
	int bulletType = BULLET_9MMH;

	if (args->bparam2)
	{
		if (args->bparam1)
		{
			gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/mp5/firesupersil.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
			bulletType = BULLET_9MMHES;
		}
		else
		{
			gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/mp5/firesuper.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
			bulletType = BULLET_9MMHE;
		}
	}
	else
	{
		if (args->bparam1)
		{
			gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/mp5/firesil.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
			bulletType = BULLET_9MMHS;
		}
		else
		{
	...
	EV_HLDM_FireBullets(idx, forward, right, up, 1, vecSrc, vecAiming, 8192, bulletType, 2, &tracerCount[idx - 1], args->fparam1, args->fparam2);
}
...
//	Con_Printf( "Firing gauss with %f\n", flDamage );
	...
	while (flDamage > 10 && nMaxHits > 0)
	{
		...
		if ( pEntity->solid == SOLID_BSP )
		{
			...
			{
				// tunnel
				EV_HLDM_DecalGunshot( &tr, BULLET_127MM );
	...
	//////////////////////////////////// WHAT TO DO HERE
							...
							EV_HLDM_DecalGunshot( &beam_tr, BULLET_127MM );

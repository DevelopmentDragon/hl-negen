...
// Little Badis: Old School weapon bobbing and strafe leaning
	...
	if (  ( g_iUser1 == OBS_IN_EYE || gHUD.m_Spectator.m_pip->value == INSET_IN_EYE ) && ent )
	{
		...
		if ( gEngfuncs.IsSpectateOnly() )
		{
			...
			if ( lastWeaponModelIndex != ent->curstate.weaponmodel )
			{
				...
				{
					gEngfuncs.pfnWeaponAnim(ent->curstate.savedvanim, ent->curstate.savedvframerate, ent->curstate.savedvframe, 1);	// reset weapon animation
				}
				else
				{
					// model not found
			...
			if ( lastViewModelIndex )
			{
				...
				gunModel->curstate.body = ent->curstate.viewmodelbody;
				gunModel->curstate.skin = ent->curstate.viewmodelskin;
				gunModel->curstate.armmodel = ent->curstate.armmodel;
				gunModel->curstate.armmodelbody = ent->curstate.armmodelbody;
				gunModel->curstate.armmodelskin = ent->curstate.armmodelskin;

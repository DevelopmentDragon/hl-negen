...
{
	// Little Badis: Was max weapons.
	for (int i = 0; i < 128; i++)
	{
		if (rgWeapons[i].iId > -1)
...
int CHudAmmo::Draw(float flTime)
{
	...
	if (m_pWeapon->iAmmoType > 0)
	{
		...
		if (pw->iClip >= 0)
		{
			...
			// draw the | bar
			FillRGBA(x, y, iBarWidth, gHUD.m_iFontHeight, 0, 1.0f, r, g, b, a, 1.0f, 1.0f, 0.0f, 0.0f, 0, 0);
...
int DrawBar(int x, int y, int width, int height, float f)
{
	...
	if (f)
	{
		...
		FillRGBA(x, y, w, height, 0, 1.0f, r, g, b, 255, 1.0f, 1.0f, 0.0f, 0.0f, 0, 0);
		x += w;
		width -= w;
	}
	...
	FillRGBA(x, y, width, height, 0, 1.0f, r, g, b, 128, 1.0f, 1.0f, 0.0f, 0.0f, 0, 0);
...
int CHudAmmo::DrawWList(float flTime)
{
	...
	for ( i = 0; i < MAX_WEAPON_SLOTS; i++ )
	{
		...
		if ( i == iActiveSlot )
		{
			...
			if (p)
				iWidth = (p->rcActive.right - p->rcActive.left) * 1.0f; //0.5f;
			else
	...
	for (i = 0; i < MAX_WEAPON_SLOTS; i++)
	{
		...
		if ( i == iActiveSlot )
		{
			...
			for ( int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++ )
			{
				...
				if ( gpActiveSel == p )
				{
					...
					SPR_DrawAdditiveScale(0, x, y, &p->rcActive, 0.5, 0.5, 0, 0, 0, 0); // x, y

					SPR_Set(gHUD.GetSprite(m_HUD_selection), r, g, b );
					SPR_DrawAdditiveScale(0, x, y, &gHUD.GetSpriteRect(m_HUD_selection), 0.5, 0.5, 0, 0, 0, 0); // x, y
				}
				else
				{
		...
		else
		{
			...
			for ( int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++ )
			{
				...
				FillRGBA( x, y, giBucketWidth, giBucketHeight, 0, 1.0f, r, g, b, a, 1.0f, 1.0f, 0.0f, 0.0f, 0, 0);

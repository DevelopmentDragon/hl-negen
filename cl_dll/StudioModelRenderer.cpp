...
// problem if we're spectating in first-person mode
		...
		{
			//gEngfuncs.Con_Printf("Viewmodel frame is %hd\n", (byte)m_pCurrentEntity->curstate.frame);
			...
			if (pTarget)
			{
				// Skin and body groups always match that of the player's viewmodelbody and viewmodelskin
				m_pCurrentEntity->curstate.body = pTarget->curstate.viewmodelbody;
				m_pCurrentEntity->curstate.skin = pTarget->curstate.viewmodelskin;

				// Insert stuff for the arm model here, we need this to ensure the models remain seperate
				m_pCurrentEntity->curstate.armmodel = pTarget->curstate.armmodel;
				m_pCurrentEntity->curstate.armmodelbody = pTarget->curstate.armmodelbody;
				m_pCurrentEntity->curstate.armmodelskin = pTarget->curstate.armmodelskin;

				//gEngfuncs.Con_Printf("%d, %d and %d\n", pTarget->curstate.armmodel, pTarget->curstate.armmodelbody, pTarget->curstate.armmodelskin);

				// RenderFX anf Render Mode synch only if they are present
				if (pTarget->curstate.renderfx > kRenderFxNone || pTarget->curstate.rendermode > kRenderNormal)
				{
					m_pCurrentEntity->curstate.renderfx = pTarget->curstate.renderfx;
					m_pCurrentEntity->curstate.rendermode = pTarget->curstate.rendermode;

...
void DLLEXPORT HUD_ProcessPlayerState( struct entity_state_s *dst, const struct entity_state_s *src )
{
	...
	dst->body = src->body;
	dst->viewmodelbody = src->viewmodelbody;
	dst->armmodelbody = src->armmodelbody;
	dst->playmodelbody = src->playmodelbody;
	dst->viewmodelskin = src->viewmodelskin;
	dst->armmodelskin = src->armmodelskin;
	dst->playmodelskin = src->playmodelskin;

	dst->savedvanim = src->savedvanim;
	dst->savedvframerate = src->savedvframerate;
	dst->savedvframe = src->savedvframe;
...
void DLLEXPORT HUD_TxferPredictionData ( struct entity_state_s *ps, const struct entity_state_s *pps, struct clientdata_s *pcd, const struct clientdata_s *ppcd, struct weapon_data_s *wd, const struct weapon_data_s *pwd )
{
	...
	ps->viewmodelbody = pps->viewmodelbody;
	ps->armmodelbody = pps->armmodelbody;
	ps->playmodelbody = pps->playmodelbody;
	ps->viewmodelskin = pps->viewmodelskin;
	ps->armmodelskin = pps->armmodelskin;
	ps->playmodelskin = pps->playmodelskin;
	ps->armmodel = pps->armmodel;
	ps->savedvanim = pps->savedvanim;
	ps->savedvframerate = pps->savedvframerate;
	ps->savedvframe = pps->savedvframe;

	pcd->viewmodel = ppcd->viewmodel;
	pcd->viewmodelbody = ppcd->viewmodelbody;
	pcd->viewmodelskin = ppcd->viewmodelskin;
	pcd->armmodel = ppcd->armmodel;
	pcd->armmodelbody = ppcd->armmodelbody;
	pcd->armmodelskin = ppcd->armmodelskin;
	pcd->playmodelbody = ppcd->playmodelbody;
	pcd->playmodelskin = ppcd->playmodelskin;
	pcd->savedvanim = ppcd->savedvanim;
	pcd->savedvframerate = ppcd->savedvframerate;
	pcd->savedvframe = ppcd->savedvframe;

	pcd->m_iId					= ppcd->m_iId;

	//for (int it = 0; it < MAX_AMMO_TYPES; it++)
	//{
		//pcd->ammo[it] = ppcd->ammo[it];
	//}
